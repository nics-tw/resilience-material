...
該表格為**依賴文件掃描**（該專案的依賴文件僅有 requirements.txt）與**全環境掃描**與其掃描出來的授權數量。全環境掃描掃出授權數量較多通常原因為該專案所使用的元件本身也使用了其他元件，形成隱性的感染，機關掃描時應盡量注意此種授權感染方式。
...
另外也有部分專案會使用某些授權的變體（如PostgreSQL License為MIT/BSD license的變體），或雙重授權機制（如MySQL和Qt使用商業授權＋GPL license），又或是將產品線區分為開源版與企業版，在無法判斷授權是否適用該案的情況下，強烈建議諮詢法律專業人士，確保充分了解所選授權的法律影響。
