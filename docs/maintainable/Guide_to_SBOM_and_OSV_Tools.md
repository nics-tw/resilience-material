...
> 備註：本文件使用 Syft v1.42.3 做說明，其使用方式及產出結果，可能因您使用的版本不同而有些微差異
...
```
$env:SYFT_FORMAT_PRETTY="true"; syft ./path/to/repo_dir -o spdx-json=./output_dir/syft.spdx.json
```
...
```
SYFT_FORMAT_PRETTY=true syft ./path/to/repo_dir -o cyclonedx-json@1.6=./output_dir/syft.cdx.json
```

Windows CMD

```
set "SYFT_FORMAT_PRETTY=true" && syft ./path/to/repo_dir -o cyclonedx-json@1.6=./output_dir/syft.cdx.json
```

Windows PowerShell

```
$env:SYFT_FORMAT_PRETTY="true"; syft ./path/to/repo_dir -o cyclonedx-json@1.6=./output_dir/syft.cdx.json
```
...
> 備註：本文件使用 Trivy v0.69.3 做說明，其使用方式及產出結果，可能因您使用的版本不同而有些微差異
...
> ⚠️ **安全警告（2026-03-19）**：Trivy 的 GitHub Actions 遭受供應鏈攻擊，v0.69.4 至 v0.69.6 版本包含惡意程式碼（CVE-2026-33634 / GHSA-69fq-xp46-6x23）。
> 請確保使用 **v0.69.3 或更早的安全版本**。GitHub Actions 使用者請注意 `trivy-action` 同樣受影響，
> 詳細資訊請參考 [Trivy 官方討論](https://github.com/aquasecurity/trivy/discussions/10425)。
...
> 備註：本文件使用 OSV-Scanner v2.3.3 做說明，其使用方式及產出結果，可能因您使用的版本不同而有些微差異

> 📌 本文件僅列出 V2（v2.0.0+）指令。若需查閱 V1（v1.x）指令說明，請參考 [OSV-Scanner V1 文件](https://google.github.io/osv-scanner-v1)。
...
```
osv-scanner scan -L ./path/to/sbom/file --format=table --output-file=report.txt
```
...
```
osv-scanner scan -L ./path/to/sbom/file --format=json --output-file=report.json
```
...
```
osv-scanner scan --format=markdown --output-file=report.md -r ./path/to/sbom_dir
```

> **注意**：`-L`（`--lockfile`）參數可同時接受 lockfile 與 SBOM 檔案，OSV-Scanner 依據副檔名自動判斷解析方式
> （如 `*.spdx.json` → SPDX SBOM、`*.cdx.json` → CycloneDX SBOM、`package-lock.json` → npm lockfile），
> 因此 SBOM 檔案必須使用標準副檔名。V1 的 `--sbom` 參數已棄用，由 `-L` 統一取代。
> 詳見 [OSV-Scanner SBOM Scanning 說明](https://google.github.io/osv-scanner/usage/scan-source#sbom-scanning)。
...
[https://google.github.io/osv-scanner/usage/offline-mode/](https://google.github.io/osv-scanner/usage/offline-mode/)

（`--experimental-offline` 與 `--experimental-download-offline-databases` 已分別穩定為 `--offline` 與 `--download-offline-databases`。）

```
osv-scanner scan --offline -L ./path/to/sbom/file --format=json --output-file=report.json
```

下載資料庫，需再加上 `--download-offline-databases`。

```
osv-scanner scan --offline --download-offline-databases -L ./path/to/sbom/file --format=json --output-file=report.json
```

或是 [手動下載](https://google.github.io/osv-scanner/usage/offline-mode/#manual-database-download) 到 [OSV資料庫資料夾](https://google.github.io/osv-scanner/usage/offline-mode/#specify-database-location)。

OSV-Scanner 內建[支援掃描](https://google.github.io/osv-scanner/supported-languages-and-lockfiles/)一些程式語言，如果專案有使用 package manager，且沒有引用 jar 或 dll，可以考慮直接使用 OSV-Scanner 就好。
...
- ./cdxgen -r "./blaze-2.1.2" -o "./blaze-sbom/sbom.json" --spec-version 1.6

參數說明：  
\-r "進行SBOM掃描的原始程式碼目錄位置"  
\-o "產生SBOM檔案放置位置"  
\--spec-version 1.6 (目前 1.6 版為目前較通用版本)  
完成後輸出畫面詳見圖 32  
![](./img/imageA30.png)  
